...
import { fetchPostsByChannel } from '../../../apis/postApi';
        ...
        if (activeTab === '베스트') {
          for (const channel of channels) {
            if (channel.name !== '전체' && channel.name !== '베스트') {
              const channelPosts = await fetchPostsByChannel(channel._id);
          ...
          if (selectedChannel) {
            const channelPosts = await fetchPostsByChannel(selectedChannel._id);
